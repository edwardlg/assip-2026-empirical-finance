...
The idea is to replace the single treatment dummy with a *set* of dummies, one for each period relative to the moment treatment switched on. Define **event time** $k = t - t^{\ast}$, where $t^{\ast}$ is the period treatment begins; $k=0$ is the first treated period, $k=-1$ is the period just before, $k=+2$ is two periods after, and so on. Then estimate
